...
ROUGE、Token Overlap、BLEU 等。通过词汇重叠率比较答案与参考文本。计算速度极快，但无法理解语义。

**2. 语义指标**

使用专用小型语言模型（如 DeBERTa-v3 NLI），比较句子对（S1, S2），输出三个维度：矛盾（S1 是否与 S2 矛盾）、蕴含（S1 是否暗示 S2）、中立（无逻辑关系）。可利用"蕴含"评估生成答案是否推导自基准答案，或利用"矛盾"查找冲突。
...
使用 600 个忠实度评估、1200 个正确性评估的人工标注数据集，通过皮尔逊、斯皮尔曼和肯德尔 Tau 三种相关性衡量标准进行测试：

- **正确性（A~A*）**：LLM 指标（尤其是 GPT-4）与人工标注相关性最高；但表现最好的确定性和语义指标可以媲美除 GPT-4 之外的其他 LLM 指标
- **忠实度（A~C）**：简单的确定性指标与 LLM 指标差距不大；LLM 指标只有在复杂提示词策略下（如将答案拆解为多个陈述，逐一评估是否可归因于上下文）才能发挥良好作用

**小结**：LLM 指标优于传统指标，但优势并不是压倒性的，简单指标同样具备有效性。
